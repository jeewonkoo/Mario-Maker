 ...
 */
FireBall::FireBall(float px, float py, Texture texture, int facing_right) : tex(texture), position({ px, py }), is_dead(false), facing_right(facing_right), alive_count(0) {
    velocity = { facing_right*0.1f, 0.1 };
}
